...
describe('Feature: Multicalendar DSO - Negative Tests', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '**/fetch_filter_values**').as('filterValues')
    cy.intercept('GET', '**/fetch_minstay_pricing_profiles**').as('minstay')
    cy.intercept('GET', '**/fetch_booking_wins_data**').as('bookingWins')
    ...
    // Wait for all background APIs to finish — this stops Chakra re-rendering
    cy.wait('@filterValues', { timeout: 15000 })
    cy.wait('@minstay', { timeout: 15000 })
    cy.wait('@bookingWins', { timeout: 15000 })
      ...
      cy.fixture('dsoData').then((data) => {
        ...
        cy.get('body').then(($body) => {
          if ($body.find('button:contains("Update")').length > 0) {
            ...
            // After going back panel should still be visible
            cy.get('p:contains("Date Specific Overrides")', { timeout: 10000 })
              .should('be.visible')
          }
        })
